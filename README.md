...
| `Fit table to window`                          | Подгоняет текущую таблицу под видимую ширину: сужает длинные ячейки или склеивает строки-продолжения обратно при расширении окна |
| `Notepad++ word wrap (MD)`                     | Включает или выключает штатный визуальный перенос строк Notepad++ рядом с кнопкой табличного переноса |
| `Table wrap (MD)`                              | Включает режим табличного переноса: при включении подгоняет текущую таблицу, затем поддерживает перенос после `Tab`/`Align table` |
| `Auto fit on resize (MD)`                      | Включает или выключает автоматическую подгонку текущей таблицы при изменении ширины окна редактора; пока включено, ручная `Fit table to window` недоступна |
...
| `Table wrap (MD)`            | тумблер в меню/на панели |
| `Auto fit on resize (MD)`    | тумблер в меню/на панели |
| `Convert CSV/TSV to table`   | `Ctrl+Alt+Shift+0` |
| `Insert table...`            | `Ctrl+Alt+Shift+\` |

### Табличный перенос и штатный перенос Notepad++

| `Table wrap (MD)` | `Перенос строк` Notepad++ | Результат |
| --- | --- | --- |
